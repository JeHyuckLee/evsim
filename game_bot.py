...
class Bot():
    ...
    def command(self, update, context):
        ...
        with conn:
            cur = conn.cursor()
            cur.execute(f"INSERT INTO game_record(chat_id, try_time, command, command_result)\
                VALUES ({update.effective_chat.id},'{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}', '{com}', '{r_text}');")
            conn.commit()
