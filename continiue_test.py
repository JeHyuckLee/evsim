...
class Generator(BehaviorModelExecutor):  #오토마타 구현
    ...
    def __init__(self, instance_time, destruct_time, name, engine_name):
        ...
        self.insert_state("SEND", 1)  # SEND상태, 1초동안 기다린후 이벤트가 없으면
    ...
    def ext_trans(self, port, msg):  #외부에서 이벤트가 들어올때 를 정의
        print(f"ext_trans{self.get_cur_state()}")
        if port == "start":  #START라는 포트에서 데이터가 들어오면 상태를 IDLE -> SEND로 변경
            print(f"[Gen][IN]: {datetime.datetime.now()}")
            self._cur_state = "MOVE"  #start를 받으면 1초단위로 SEND 라는 상태를 반복

    def output(
        self
    ):  # 메세지를 보냄, 프로세스를 지정, 프로세스포트와 연결된 에이전트는 전부 메세지를 받는다. 게임을 만드는 사람은 누가 메세지를받을지 포트를 지정
        print(f"output{self.get_cur_state()}")
        msg = SysMessage(self.get_name(), "process")  #SEND - 1초주기로 메세지를 보냄
        print(f"[Gen][OUT]: {datetime.datetime.now()}")
        msg.insert(self.msg_list.pop(0))  #메세지 리스트에서 하나를 뽑아서 보낸다.
        return msg

    def int_trans(self):
        print(self.get_cur_state())
        if self._cur_state == "SEND" and not self.msg_list:  #SEND 상태, 메세지가 비어있는경우 IDLE상태로 가라
...
se.register_engine("sname", "REAL_TIME",
